    ...
    if ((direc = opendir (path)) != NULL) 
    {
      /* print all the files and directories within directory */
        while ((ent = readdir (direc)) != NULL)
        {
            ...
            //printf ("%s\n", ent->d_name);
            if(strcasecmp(ent->d_name,"index.html")==0)
            {
                strcat(s,"/index.html");
                return s; 
            }
            else if(strcasecmp(ent->d_name,"index.php")==0)
            {
                strcat(s,"/index.php");
                return s; 
            }
...
bool parse(const char* line, char* abs_path, char* query)
{
  ...
  if (strncmp(line, "GET ", 4) != 0)
    ...
    char* reqstrt = strchr(line, ' ') + 1; 		
    char* reqend = version - 1;                
    size_t len1 = reqend - reqstrt;			
    char* req = malloc(sizeof(char)*(len1+1));
    req[0]='\0';  
    strncpy(req, reqstrt, len1);				
     req[len1]='\0'; //append the null character since strncpy does not null terminate
    
    char* querystart=strchr(line, '?');
    if((querystart)!=NULL){
 
        size_t len2 = querystart - reqstrt;
        strncpy(abs_path, req, len2);
        abs_path[len2]='\0';
        
        if(!isspace(querystart[1])){
 
            size_t len3 = reqend - (querystart + 1); 
 
            if(len3>=1){
                strncpy(query, querystart+1,len3);
                 query[len3]='\0';
            }
        }
        free(req);
